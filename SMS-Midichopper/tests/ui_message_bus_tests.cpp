...
void independentReadersAndWrap()
{
    ...
    check(bus.read(first, message) && std::strcmp(message.value.data(), "one") == 0,
          "first reader receives first message");
    check(bus.read(second, message) && std::strcmp(message.value.data(), "one") == 0,
          "second reader receives same first message");
    check(bus.read(first, message) && std::strcmp(message.key.data(), "chop_status") == 0,
          "first reader receives second message in order");
    check(bus.read(second, message) && std::strcmp(message.value.data(), "two") == 0,
          "second reader receives second message");
    check(!bus.read(first, message), "reader stops at current end");

    for (std::size_t i = 0; i < bus.kCapacity + 5U; ++i)
        check(bus.publish("status", std::to_string(i).c_str()), "publish ring message");
    check(bus.read(first, message) && std::strcmp(message.value.data(), "5") == 0,
...
void boundsAndRealPayload()
{
    ...
    auto reader = initial;
    midichopper::plugin::UiMessageBus::Message message;
    check(bus.read(reader, message) && std::strcmp(message.value.data(), encoded.c_str()) == 0,
